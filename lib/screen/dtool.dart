...
import 'package:flutter_aten/page/EditdtoolmPage.dart';
import 'package:flutter_aten/providers/user_provider.dart';
import 'package:flutter_aten/screen/AdminPage.dart';
import 'package:flutter_aten/screen/droom.dart';
import 'package:provider/provider.dart';
import '../widget/customCliper.dart';

class DtoolPage extends StatefulWidget {
  const DtoolPage({super.key});

  @override
  State<DtoolPage> createState() => _DtoolPageState();
}

class _DtoolPageState extends State<DtoolPage> {
  int currentPageIndex = 0;
  List<DtoolModeel> dtooms = []; // แก้ไขชื่อเป็น RoomModel
  bool isLoading = true;
  String? errorMessage;
  ...
  Future<void> _fetchRooms() async {
    try {
      final roomList = await DtoolController().getdtools(context);
      setState(() {
        dtooms = roomList;
        isLoading = false;
      });
  ...
  void updatedroom(DtoolModeel dtool) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => EditdtoolPage(dtool: dtool),
      ),
    );
  }

  Future<void> deletedroom(DtoolModeel tool) async {
    ...
    if (confirmDelete == true) {
      try {
        final response = await DtoolController().deletedtool(context, tool.id);
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () {
                        Navigator.pushNamed(context, '/insertdtool');
                      },
                      style: ElevatedButton.styleFrom(
                          backgroundColor: const Color(0xff821131)),
                      child: Text('เพิ่มtool',
                          style: TextStyle(color: Colors.white)),
                    ),
                    SizedBox(height: 20),
                    if (isLoading)
  ...
  Widget _buildRoomList() {
    return Column(
      children: List.generate(dtooms.length, (index) {
        final dtoomItem = dtooms[index];
        return Container(
          ...
          child: ListTile(
            title: Text(dtoomItem.id),
            subtitle: Text('ID: ${dtoomItem.id}'),
            trailing: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                IconButton(
                  icon: Icon(Icons.edit),
                  onPressed: () => updatedroom(dtoomItem),
                ),
                IconButton(
                  icon: Icon(Icons.delete),
                  onPressed: () => deletedroom(dtoomItem),
                ),
              ],
            ),
          ),
